...
import { RESOURCE_CHEST_POSITION, SEEDS_CHEST_POSITION } from "../utils/constants";
...
const searchItem: ObjectName = "SwitchGrass";
const toolsAvailble = ["DiamondPick"] as ObjectName[];
const maxTotalTools = 10;
const waitForTxn = true;

export async function collectSeedsBot(context: BotContext) {
  ...
  await InteractWithChest({ objectName: "WheatSeed", chestCoord: SEEDS_CHEST_POSITION, amount: 10e8, action: "deposit" }, context);
  await InteractWithChest({ objectName: "MelonSeed", chestCoord: SEEDS_CHEST_POSITION, amount: 10e8, action: "deposit" }, context);
  await InteractWithChest({ objectName: "PumpkinSeed", chestCoord: SEEDS_CHEST_POSITION, amount: 10e8, action: "deposit" }, context);
